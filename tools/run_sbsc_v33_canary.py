#!/usr/bin/env python3
"""Run the immutable SCTransNet C3-SBSC V3.3 train-only canary.
...
CANARY_RULES_PATH = PROJECT_ROOT / "experiments" / "sbsc_v33_canary_rules.json"
EXPECTED_RULES_SHA256 = (
    "0f5df664a03e99087ce426f700a32081495a439ae6f2cc41b23bbda3f829815f"
)
EXPECTED_METHOD_CONFIG_SHA256 = (
    "0f84d9dc3304256e303249eb245b7a403181145bd1e691ec106ba1773d9a58b1"
)
RULES_SCHEMA = "sctransnet_sbsc_v33/canary_rules/v1"
METHOD_CONFIG_SCHEMA = "sctransnet_sbsc_v33/canary_method_config/v1"
MANIFEST_SCHEMA = "sctransnet_sbsc_v33/canary_manifest/v1"
REPORT_SCHEMA = "sctransnet_sbsc_v33/canary_report/v1"
...
OUTPUT_NAMESPACE = "runs/sbsc_v33_third/canary/IRSTD-1K"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the fixed V3.3 IRSTD-1K train-only canary"
    )
...
def _audit_combined_backward(
    ...
    gain_ok = (
        gain_name in named
        and named[gain_name].grad is not None
        and bool(torch.isfinite(named[gain_name].grad.detach()).all())
    )
    ...
    if missing or nonfinite or nonzero_elements <= 0 or not router_ok or not gain_ok or not value_ok:
        _fail(
            "combined_backward",
...
def _execute_training(prepared: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for images, masks in loader:
            ...
            total_loss.backward()
            gradient = _audit_combined_backward(model, active_names, value_spatial)
            optimizer.step()
...
def run_canary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        epochs = _execute_training(prepared)
        ...
        report = {
            **_report_common(manifest_sha, prepared),
            ...
            "processed_samples": sum(record["processed_samples"] for record in epochs),
            "optimizer_steps": sum(record["optimizer_steps"] for record in epochs),
