"""Frozen validation-only dual-role selection for SCTransNet/SBSC V2.1.
...
SELECTION_SCHEMA = "sctransnet_sbsc_v21_validation_dual_role/v1"
SCHEDULE_SCHEMA = "sctransnet_sbsc_v21_validation_schedule/v1"
...
def _sha256_text(value: Any, *, label: str) -> str:
    if (
        ...
        raise SBSCV21SelectionError(f"{label} must be lowercase SHA-256")
    return value
...
def _validated_identity(value: Any) -> dict[str, Any]:
    ...
    if identity["method"] not in _METHODS:
        raise SBSCV21SelectionError("expected_identity method differs")
    if identity["dataset"] not in _DATASETS:
        raise SBSCV21SelectionError("expected_identity dataset differs")
    for field in ("architecture_seed", "run_seed"):
        if type(identity[field]) is not int or identity[field] != 42:
            raise SBSCV21SelectionError(f"expected_identity {field} must be 42")
    ...
    *,
    ...
    if materialized:
        try:
            ...
            raise SBSCV21SelectionError(str(exc)) from exc
    return materialized
...
def select_final(
    ...
    if payload["validation_record_count"] != expected_count:
        raise SBSCV21SelectionError("final validation history is incomplete")
    if set(payload["roles"]) != set(_FROZEN_VALID_ROLES):
        raise SBSCV21SelectionError("final dual-role selection is incomplete")
    return payload
