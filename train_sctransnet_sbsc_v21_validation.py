#!/usr/bin/env python3
"""Train SCTransNet or SBSC-V2.1 with fixed-seed V2.1 validation selection.
...
from experiments import sbsc_v21_selection as selection
from experiments.evisirst_v2_data import DEFAULT_SPLIT_ROOT, V2SplitContract


PROJECT_ROOT = Path(__file__).resolve().parent
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "runs" / "sctransnet_sbsc_v21_validation"
METHODS = ("sctransnet", "sbsc_v21")
ARCHITECTURE_SEED = 42
RUN_SEED = 42
SHUFFLE_STREAM = "sctransnet_sbsc_v2_pair"
...
EXPECTED_SBSC_V21_STATE_KEY_COUNT = 511
EXPECTED_SBSC_V21_PARAMETER_COUNT = 11_325_940
...
TRAINING_SCHEMA = "sctransnet_sbsc_v21_validation_training/v1"
CANDIDATE_SCHEMA = "sctransnet_sbsc_v21_validation_candidate/v1"
CHECKPOINT_SCHEMA = "sctransnet_sbsc_v21_validation_checkpoint/v1"
HISTORY_SCHEMA = "sctransnet_sbsc_v21_validation_history/v1"
SOURCE_SET_SCHEMA = "sctransnet_sbsc_v21_validation_source_set/v1"
SMOKE_SELECTION_SCHEMA = "sctransnet_sbsc_v21_smoke_selection/v1"
...
class SCTransNetSBSCV21RunnerError(r1.ValidationSelectedTrainingError):
...
def _strict_json_clone(value: Any, *, label: str) -> Any:
    try:
        ...
        raise SCTransNetSBSCV21RunnerError(
...
def _state_dict_sha256(state: Mapping[str, torch.Tensor]) -> str:
    ...
    if not isinstance(state, Mapping):
        raise SCTransNetSBSCV21RunnerError("state hash input must be a mapping")
    digest = hashlib.sha256()
    for key in sorted(state):
        value = state[key]
        if not isinstance(key, str) or not isinstance(value, torch.Tensor):
            raise SCTransNetSBSCV21RunnerError("state hash input is malformed")
...
def _selection_identity(identity: Mapping[str, Any]) -> dict[str, Any]:
    ...
    except selection.SBSCV21SelectionError as exc:
        raise SCTransNetSBSCV21RunnerError(str(exc)) from exc
...
def resolve_run_paths(args: argparse.Namespace) -> dict[str, Path | bool]:
    ...
    if args.method not in METHODS:
        raise SCTransNetSBSCV21RunnerError("unsupported method")
...
def validation_begin_epoch(total_epochs: int, *, smoke: bool) -> int:
    if isinstance(total_epochs, bool) or not isinstance(total_epochs, int):
        raise SCTransNetSBSCV21RunnerError("total epochs must be an integer")
    if not 1 <= total_epochs <= FORMAL_EPOCHS:
        raise SCTransNetSBSCV21RunnerError("total epochs are outside the contract")
...
def training_shuffle_seed(dataset: str, epoch: int) -> int:
    ...
    if dataset not in v2_data.source_protocol.DATASETS:
        raise SCTransNetSBSCV21RunnerError("shuffle dataset is unsupported")
    if (
        isinstance(epoch, bool)
        or not isinstance(epoch, int)
        or not 1 <= epoch <= FORMAL_EPOCHS
    ):
        raise SCTransNetSBSCV21RunnerError("shuffle epoch is outside [1, 1000]")
...
def training_shuffle_generator(args: argparse.Namespace, epoch: int) -> torch.Generator:
    ...
    if args.method not in METHODS:
        raise SCTransNetSBSCV21RunnerError("shuffle method is unsupported")
...
def expected_validation_epochs(
    ...
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("completed epoch is outside the run")
    ...
    *,
    ...
    if not smoke:
        if total_epochs != FORMAL_EPOCHS:
            raise SCTransNetSBSCV21RunnerError("formal selector requires 1000 epochs")
...
def _select_final(
    ...
    *,
    ...
    if set(payload["roles"]) != set(zero_selection.VALID_ROLES):
        raise SCTransNetSBSCV21RunnerError("smoke dual-role selection is incomplete")
    return payload
...
def _load_core_module() -> ModuleType:
    ...
    try:
        module = importlib.import_module("experiments.sctransnet_sbsc_v21")
    except (ImportError, ModuleNotFoundError) as exc:
        raise SCTransNetSBSCV21RunnerError(
            "SBSC-V2.1 core builder is unavailable; training was not started"
        ) from exc
    required = (
    ...
    if any(not callable(getattr(module, name, None)) for name in required):
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 core API is incomplete")
    source = Path(module.__file__ or "")
    if source.is_symlink() or not source.is_file():
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 core source is not regular")
    try:
        source.resolve(strict=True).relative_to(PROJECT_ROOT.resolve(strict=True))
    except ValueError as exc:
        raise SCTransNetSBSCV21RunnerError(
            "SBSC-V2.1 core source is outside the repository"
        ) from exc
    return module
...
def _gain_contract(
    ...
    if method == "sctransnet":
        if actual_gain_keys:
            raise SCTransNetSBSCV21RunnerError("baseline unexpectedly contains gains")
    ...
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 gain state-key contract differs")
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 gain bounds differ from [0, 0.5]")
    for key in GAIN_STATE_KEYS:
        tensor = expected_state[key]
        if not tensor.is_floating_point() or tensor.numel() != 1:
            raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 gains must be scalar floats")
...
def _state_contract(
    ...
    expected_key_count = (
        ...
        if method == "sctransnet"
        else EXPECTED_SBSC_V21_STATE_KEY_COUNT
    )
    expected_parameter_count = (
        EXPECTED_BASELINE_PARAMETER_COUNT
        if method == "sctransnet"
        else EXPECTED_SBSC_V21_PARAMETER_COUNT
    )
...
def _freeze_structurally_inactive(
    ...
    if isinstance(raw_names, (str, bytes, Mapping)):
        raise SCTransNetSBSCV21RunnerError("inactive parameter names are malformed")
    try:
    ...
    if not set(names).issubset(named):
        raise SCTransNetSBSCV21RunnerError("inactive parameters are absent from model")
    for name in names:
        named[name].requires_grad_(False)
    if any(named[name].requires_grad for name in names):
        raise SCTransNetSBSCV21RunnerError("inactive parameter freezing failed")
...
def build_model(
    ...
    try:
        result = core.build_sctransnet_sbsc_v21_method(
            method=args.method,
            dataset=args.dataset,
            architecture_seed=ARCHITECTURE_SEED,
            training=True,
        ...
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 method builder failed") from exc
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("SBSC-V2.1 method builder returned malformed data")
    ...
    if metadata.get("method", args.method) != args.method:
        raise SCTransNetSBSCV21RunnerError("builder method identity differs")
    if args.method == "sbsc_v21":
        try:
            core.validate_sctransnet_sbsc_v21(model, require_zero_gain=True)
        except Exception as exc:
            raise SCTransNetSBSCV21RunnerError(
                "fresh SBSC-V2.1 model violates its zero-gain contract"
            ) from exc
...
def _validate_serialized_gains_before_load(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise SCTransNetSBSCV21RunnerError("checkpoint state is not a mapping")
    gain_keys = tuple(gain_contract.get("state_keys", ()))
    if method == "sctransnet":
        if any("gain" in str(key) for key in value):
            raise SCTransNetSBSCV21RunnerError("baseline checkpoint contains a gain")
        return
    if set(gain_keys) != set(GAIN_STATE_KEYS):
        raise SCTransNetSBSCV21RunnerError("trusted gain contract is malformed")
    for key in gain_keys:
        ...
        numeric = float(tensor.detach().cpu().reshape(()).item())
        if not GAIN_MIN <= numeric <= GAIN_MAX:
            raise SCTransNetSBSCV21RunnerError(
                f"serialized gain is outside [0, 0.5]: {key}"
            )
...
def _validate_state_dict(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != set(expected):
        raise SCTransNetSBSCV21RunnerError("checkpoint state keys differ")
...
def _load_model_state(
    ...
    *,
    ...
    try:
        core.validate_sbsc_v21_state_dict(value, method)
    except Exception as exc:
        raise SCTransNetSBSCV21RunnerError(
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise SCTransNetSBSCV21RunnerError("strict model state load failed")
    if method == "sbsc_v21":
        try:
            core.validate_sctransnet_sbsc_v21(model, require_zero_gain=False)
        except Exception as exc:
            raise SCTransNetSBSCV21RunnerError(
                "loaded SBSC-V2.1 model violates its architecture contract"
            ) from exc
    return state
...
def _source_provenance(core: ModuleType) -> dict[str, Any]:
    ...
    if not internal_paths:
        raise SCTransNetSBSCV21RunnerError("model internal source tree is missing")
...
def _run_identity(
    ...
    *,
    ...
    identity: dict[str, Any] = {
        ...
        "model": "SCTransNet" if args.method == "sctransnet" else "SCTransNet-SBSC-V2.1",
        "method": args.method,
        "dataset": args.dataset,
        "architecture_seed": ARCHITECTURE_SEED,
        "run_seed": RUN_SEED,
    ...
    *,
    ...
    try:
        ...
        if set(bound_identity) != {
            ...
            raise SCTransNetSBSCV21RunnerError("selection identity fields differ")
        if (
            ...
            raise SCTransNetSBSCV21RunnerError("model state SHA-256 is malformed")
        record = {
            "epoch": int(epoch),
            "data_role": "val",
...
def _candidate_path(candidate_dir: Path, epoch: int) -> Path:
    if isinstance(epoch, bool) or not isinstance(epoch, int) or epoch < 1:
        raise SCTransNetSBSCV21RunnerError("candidate epoch must be positive")
    return candidate_dir / f"epoch_{epoch:04d}.pth.tar"


def _regular_file(path: Path, *, run_dir: Path, label: str) -> Path:
    if path.is_symlink() or not path.is_file():
        raise SCTransNetSBSCV21RunnerError(f"{label} is not a regular file")
...
def _candidate_epoch(path: Path, candidate_dir: Path) -> int:
    ...
    if path.name != _candidate_path(candidate_dir, epoch).name:
        raise SCTransNetSBSCV21RunnerError("candidate filename is not canonical")
    return epoch
...
def _validate_candidate_payload(
    *,
    ...
    try:
        ...
        raise SCTransNetSBSCV21RunnerError("candidate could not be read safely") from exc
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("candidate identity differs")
    record = dict(payload["validation_record"])
    if record.get("epoch") != epoch:
        raise SCTransNetSBSCV21RunnerError("candidate validation epoch differs")
    try:
        zero_selection.select_checkpoints([record])
    except zero_selection.EviSIRSTZeroMarginSelectionError as exc:
        raise SCTransNetSBSCV21RunnerError(str(exc)) from exc
    if expected_record is not None and record != expected_record:
        raise SCTransNetSBSCV21RunnerError("candidate validation record differs")
...
def _frontier_artifacts(
    *,
    ...
    for epoch in frontier:
        ...
        if path.is_symlink() or not path.is_file():
            raise SCTransNetSBSCV21RunnerError("frontier candidate is missing")
        artifacts[epoch] = {
...
def _validate_histories(
    *,
    ...
    if not isinstance(training_history, list) or len(training_history) != completed_epoch:
        raise SCTransNetSBSCV21RunnerError("resume training history is incomplete")
    ...
    for expected_epoch, raw in enumerate(training_history, start=1):
        if not isinstance(raw, Mapping) or raw.get("epoch") != expected_epoch:
            raise SCTransNetSBSCV21RunnerError("resume training history order differs")
        record = _strict_json_clone(dict(raw), label="training history")
        for field in ("mean_train_loss", "learning_rate"):
            value = record.get(field)
            if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
                raise SCTransNetSBSCV21RunnerError("training history is non-finite")
        if float(record["mean_train_loss"]) < 0.0:
            raise SCTransNetSBSCV21RunnerError("training loss must be non-negative")
    ...
    if not isinstance(validation_history, list):
        raise SCTransNetSBSCV21RunnerError("resume validation history is malformed")
...
def _normalize_artifact_map(value: Any) -> dict[int, dict[str, Any]]:
    if not isinstance(value, Mapping):
        raise SCTransNetSBSCV21RunnerError("candidate artifact map is malformed")
    normalized: dict[int, dict[str, Any]] = {}
    for raw_epoch, raw_artifact in value.items():
        if isinstance(raw_epoch, bool) or not isinstance(raw_epoch, int):
            raise SCTransNetSBSCV21RunnerError("candidate artifact epoch is malformed")
        if not isinstance(raw_artifact, Mapping):
            raise SCTransNetSBSCV21RunnerError("candidate artifact is malformed")
...
def _validate_resume_candidates(
    *,
    ...
    if tuple(sorted(normalized)) != frontier:
        raise SCTransNetSBSCV21RunnerError("resume artifacts differ from dual frontier")
    ...
    for epoch in frontier:
        ...
        if artifact.get("relative_path") != f"candidates/{path.name}":
            raise SCTransNetSBSCV21RunnerError("candidate relative path differs")
        path = _regular_file(path, run_dir=run_dir, label="frontier candidate")
        expected_files.add(path)
        if artifact.get("file_sha256") != _sha256_file(path):
            raise SCTransNetSBSCV21RunnerError("candidate hash differs")
    ...
    if candidate_dir.exists():
        if candidate_dir.is_symlink() or not candidate_dir.is_dir():
            raise SCTransNetSBSCV21RunnerError("candidate directory is not regular")
...
def _prune_after_committed_latest(
    *,
    ...
    for path in candidate_dir.iterdir():
        if path.is_symlink() or not path.is_file():
            raise SCTransNetSBSCV21RunnerError("candidate entry is not regular")
        epoch = _candidate_epoch(path, candidate_dir)
...
def _load_resume_state(
    *,
    ...
    try:
        ...
        raise SCTransNetSBSCV21RunnerError("resume state could not be read safely") from exc
    if (
        not isinstance(payload, Mapping)
        ...
        raise SCTransNetSBSCV21RunnerError("resume run identity differs")
    ...
    if (
        ...
        raise SCTransNetSBSCV21RunnerError("resume epoch is outside the run")
    _load_model_state(
        model,
        payload.get("state_dict"),
        method=str(identity["method"]),
    ...
    try:
        ...
        raise SCTransNetSBSCV21RunnerError("resume RNG state is malformed") from exc
...
def _load_candidate_state(
    *,
    ...
    if epoch not in artifacts:
        raise SCTransNetSBSCV21RunnerError("selected epoch is absent from frontier")
    path = _regular_file(
    ...
    if artifacts[epoch].get("file_sha256") != _sha256_file(path):
        raise SCTransNetSBSCV21RunnerError("selected candidate hash differs")
    record = next(
        (record for record in history if int(record["epoch"]) == epoch), None
    )
    if record is None:
        raise SCTransNetSBSCV21RunnerError("selected validation record is missing")
    _validate_candidate_payload(
        path=path,
        run_dir=run_dir,
        candidate_dir=candidate_dir,
...
def _final_checkpoint_payload(
    *,
    ...
    if role not in zero_selection.VALID_ROLES:
        raise SCTransNetSBSCV21RunnerError("invalid final checkpoint role")
...
def _require_selected_state_hash(
    ...
    if role not in zero_selection.VALID_ROLES:
        raise SCTransNetSBSCV21RunnerError("invalid selected-state role")
    try:
...
def _atomic_write_final_pair(
    *,
    ...
    if set(payloads) != expected_roles:
        raise SCTransNetSBSCV21RunnerError("exactly two final role payloads are required")
    allowed_names = {
    ...
    if unexpected:
        raise SCTransNetSBSCV21RunnerError("unexpected final weight file exists")
    ...
    for role in zero_selection.VALID_ROLES:
        ...
        if (
            ...
            raise SCTransNetSBSCV21RunnerError("final checkpoint verification failed")
        finals[role] = {
    ...
    if actual != allowed_names:
        raise SCTransNetSBSCV21RunnerError("final physical weight set is not exact")
    return finals
...
def _process_lock(path: Path) -> Iterator[None]:
    ...
    if path.is_symlink():
        raise SCTransNetSBSCV21RunnerError("run lock must not be a symlink")
    ...
    try:
        try:
            ...
            raise SCTransNetSBSCV21RunnerError("this run is already active") from exc
        yield
    finally:
    ...
    *,
    ...
    if validation_record is not None:
        if int(validation_record["epoch"]) != epoch:
            raise SCTransNetSBSCV21RunnerError("validation record epoch differs")
        _save_candidate(
            path=_candidate_path(candidate_dir, epoch),
            epoch=epoch,
            state=state,
...
def _run_locked(
    ...
    if len(train_data) < 1 or len(val_data) < 1:
        raise SCTransNetSBSCV21RunnerError("train/validation data is empty")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for images, masks in train_loader:
            ...
            loss = r1.legacy_train.deep_supervision_loss(
            ...
            optimizer.step()
            if args.method == "sbsc_v21":
                core.project_sbsc_v21_constraints_(model)
        ...
        print(
            ...
            f" loss={mean_loss:.6f} lr={learning_rate:.8f}{validation_text}",
            flush=True,
        )
    ...
    if tuple(sorted(candidate_artifacts)) != frontier:
        raise SCTransNetSBSCV21RunnerError("final artifacts differ from dual frontier")
...
def run(args: argparse.Namespace) -> dict[str, Path]:
    if args.architecture_seed != ARCHITECTURE_SEED or args.run_seed != RUN_SEED:
        raise SCTransNetSBSCV21RunnerError("architecture and runtime seed must be 42")
    if args.preflight:
        raise SCTransNetSBSCV21RunnerError(
