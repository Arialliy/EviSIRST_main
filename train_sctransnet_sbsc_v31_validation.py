#!/usr/bin/env python3
"""Train SCTransNet or C3-SBSC V3.1 with fixed-seed validation selection.
...
from experiments import sbsc_v31_selection as selection
from experiments.evisirst_v2_data import DEFAULT_SPLIT_ROOT, V2SplitContract


PROJECT_ROOT = Path(__file__).resolve().parent
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "runs" / "sctransnet_sbsc_v31_validation"
METHODS = ("sctransnet", "sbsc_v31")
...
EXPECTED_SBSC_V31_STATE_KEY_COUNT = 511
EXPECTED_SBSC_V31_PARAMETER_COUNT = 11_325_943
...
TRAINING_SCHEMA = "sctransnet_sbsc_v31_validation_training/v1"
CANDIDATE_SCHEMA = "sctransnet_sbsc_v31_validation_candidate/v1"
CHECKPOINT_SCHEMA = "sctransnet_sbsc_v31_validation_checkpoint/v1"
HISTORY_SCHEMA = "sctransnet_sbsc_v31_validation_history/v1"
SOURCE_SET_SCHEMA = "sctransnet_sbsc_v31_validation_source_set/v1"
SMOKE_SELECTION_SCHEMA = "sctransnet_sbsc_v31_smoke_selection/v1"
...
class SCTransNetSBSCV31RunnerError(r1.ValidationSelectedTrainingError):
...
def _strict_json_clone(value: Any, *, label: str) -> Any:
    try:
        ...
        raise SCTransNetSBSCV31RunnerError(
...
def _state_dict_sha256(state: Mapping[str, torch.Tensor]) -> str:
    ...
    if not isinstance(state, Mapping):
        raise SCTransNetSBSCV31RunnerError("state hash input must be a mapping")
    digest = hashlib.sha256()
    for key in sorted(state):
        value = state[key]
        if not isinstance(key, str) or not isinstance(value, torch.Tensor):
            raise SCTransNetSBSCV31RunnerError("state hash input is malformed")
...
def _selection_identity(identity: Mapping[str, Any]) -> dict[str, Any]:
    ...
    except selection.SBSCV31SelectionError as exc:
        raise SCTransNetSBSCV31RunnerError(str(exc)) from exc
...
def resolve_run_paths(args: argparse.Namespace) -> dict[str, Path | bool]:
    ...
    if args.method not in METHODS:
        raise SCTransNetSBSCV31RunnerError("unsupported method")
...
def validation_begin_epoch(total_epochs: int, *, smoke: bool) -> int:
    if isinstance(total_epochs, bool) or not isinstance(total_epochs, int):
        raise SCTransNetSBSCV31RunnerError("total epochs must be an integer")
    if not 1 <= total_epochs <= FORMAL_EPOCHS:
        raise SCTransNetSBSCV31RunnerError("total epochs are outside the contract")
...
def training_shuffle_seed(dataset: str, epoch: int) -> int:
    ...
    if dataset not in v2_data.source_protocol.DATASETS:
        raise SCTransNetSBSCV31RunnerError("shuffle dataset is unsupported")
    if (
        isinstance(epoch, bool)
        or not isinstance(epoch, int)
        or not 1 <= epoch <= FORMAL_EPOCHS
    ):
        raise SCTransNetSBSCV31RunnerError("shuffle epoch is outside [1, 1000]")
...
def training_shuffle_generator(args: argparse.Namespace, epoch: int) -> torch.Generator:
    ...
    if args.method not in METHODS:
        raise SCTransNetSBSCV31RunnerError("shuffle method is unsupported")
...
def expected_validation_epochs(
    ...
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("completed epoch is outside the run")
    ...
    *,
    ...
    if not smoke:
        if total_epochs != FORMAL_EPOCHS:
            raise SCTransNetSBSCV31RunnerError("formal selector requires 1000 epochs")
...
def _select_final(
    ...
    *,
    ...
    if set(payload["roles"]) != set(zero_selection.VALID_ROLES):
        raise SCTransNetSBSCV31RunnerError("smoke dual-role selection is incomplete")
    return payload
...
def _load_core_module() -> ModuleType:
    ...
    try:
        module = importlib.import_module("experiments.sctransnet_sbsc_v31")
    except (ImportError, ModuleNotFoundError) as exc:
        raise SCTransNetSBSCV31RunnerError(
            "SBSC-V3.1 core builder is unavailable; training was not started"
        ) from exc
    required = (
    ...
    if any(not callable(getattr(module, name, None)) for name in required):
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 core API is incomplete")
    source = Path(module.__file__ or "")
    if source.is_symlink() or not source.is_file():
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 core source is not regular")
    try:
        source.resolve(strict=True).relative_to(PROJECT_ROOT.resolve(strict=True))
    except ValueError as exc:
        raise SCTransNetSBSCV31RunnerError(
            "SBSC-V3.1 core source is outside the repository"
        ) from exc
    return module
...
def _gain_contract(
    ...
    if method == "sctransnet":
        if actual_gain_keys:
            raise SCTransNetSBSCV31RunnerError("baseline unexpectedly contains gains")
    ...
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 gain state-key contract differs")
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 gain bounds differ from [0, 0.25]")
    for key in GAIN_STATE_KEYS:
        tensor = expected_state[key]
        if tuple(tensor.shape) != (4,) or tensor.dtype is not torch.float32:
            raise SCTransNetSBSCV31RunnerError(
                "SBSC-V3.1 gain must be one FP32 vector [4]"
            )
...
def _state_contract(
    ...
    expected_key_count = (
        ...
        if method == "sctransnet"
        else EXPECTED_SBSC_V31_STATE_KEY_COUNT
    )
    expected_parameter_count = (
        EXPECTED_BASELINE_PARAMETER_COUNT
        if method == "sctransnet"
        else EXPECTED_SBSC_V31_PARAMETER_COUNT
    )
...
def _freeze_structurally_inactive(
    ...
    if isinstance(raw_names, (str, bytes, Mapping)):
        raise SCTransNetSBSCV31RunnerError("inactive parameter names are malformed")
    try:
    ...
    if not set(names).issubset(named):
        raise SCTransNetSBSCV31RunnerError("inactive parameters are absent from model")
    for name in names:
        named[name].requires_grad_(False)
    if any(named[name].requires_grad for name in names):
        raise SCTransNetSBSCV31RunnerError("inactive parameter freezing failed")
...
def build_model(
    ...
    try:
        result = core.build_sctransnet_sbsc_v31_method(
            method=args.method,
            dataset=args.dataset,
            architecture_seed=ARCHITECTURE_SEED,
            training=True,
        ...
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 method builder failed") from exc
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("SBSC-V3.1 method builder returned malformed data")
    ...
    if metadata.get("method", args.method) != args.method:
        raise SCTransNetSBSCV31RunnerError("builder method identity differs")
    if args.method == "sbsc_v31":
        try:
            core.validate_sctransnet_sbsc_v31(model, require_zero_gain=True)
        except Exception as exc:
            raise SCTransNetSBSCV31RunnerError(
                "fresh SBSC-V3.1 model violates its zero-gain contract"
            ) from exc
...
def _validate_serialized_gains_before_load(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise SCTransNetSBSCV31RunnerError("checkpoint state is not a mapping")
    gain_keys = tuple(gain_contract.get("state_keys", ()))
    if method == "sctransnet":
        if any("gain" in str(key) for key in value):
            raise SCTransNetSBSCV31RunnerError("baseline checkpoint contains a gain")
        return
    if set(gain_keys) != set(GAIN_STATE_KEYS):
        raise SCTransNetSBSCV31RunnerError("trusted gain contract is malformed")
...
def _validate_state_dict(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != set(expected):
        raise SCTransNetSBSCV31RunnerError("checkpoint state keys differ")
...
def _load_model_state(
    ...
    *,
    ...
    try:
        core.validate_sbsc_v31_state_dict(value, method)
    except Exception as exc:
        raise SCTransNetSBSCV31RunnerError(
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise SCTransNetSBSCV31RunnerError("strict model state load failed")
    if method == "sbsc_v31":
        try:
            core.validate_sctransnet_sbsc_v31(model, require_zero_gain=False)
        except Exception as exc:
            raise SCTransNetSBSCV31RunnerError(
                "loaded SBSC-V3.1 model violates its architecture contract"
            ) from exc
    return state
...
def _source_provenance(core: ModuleType) -> dict[str, Any]:
    ...
    if not internal_paths:
        raise SCTransNetSBSCV31RunnerError("model internal source tree is missing")
    ...
    *,
    ...
    try:
        ...
        if set(bound_identity) != {
            ...
            raise SCTransNetSBSCV31RunnerError("selection identity fields differ")
        if (
            ...
            raise SCTransNetSBSCV31RunnerError("model state SHA-256 is malformed")
        record = {
            "epoch": int(epoch),
            "data_role": "val",
...
def _candidate_path(candidate_dir: Path, epoch: int) -> Path:
    if isinstance(epoch, bool) or not isinstance(epoch, int) or epoch < 1:
        raise SCTransNetSBSCV31RunnerError("candidate epoch must be positive")
    return candidate_dir / f"epoch_{epoch:04d}.pth.tar"


def _regular_file(path: Path, *, run_dir: Path, label: str) -> Path:
    if path.is_symlink() or not path.is_file():
        raise SCTransNetSBSCV31RunnerError(f"{label} is not a regular file")
...
def _candidate_epoch(path: Path, candidate_dir: Path) -> int:
    ...
    if path.name != _candidate_path(candidate_dir, epoch).name:
        raise SCTransNetSBSCV31RunnerError("candidate filename is not canonical")
    return epoch
...
def _validate_candidate_payload(
    *,
    ...
    try:
        ...
        raise SCTransNetSBSCV31RunnerError("candidate could not be read safely") from exc
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("candidate identity differs")
    record = dict(payload["validation_record"])
    if record.get("epoch") != epoch:
        raise SCTransNetSBSCV31RunnerError("candidate validation epoch differs")
    try:
        zero_selection.select_checkpoints([record])
    except zero_selection.EviSIRSTZeroMarginSelectionError as exc:
        raise SCTransNetSBSCV31RunnerError(str(exc)) from exc
    if expected_record is not None and record != expected_record:
        raise SCTransNetSBSCV31RunnerError("candidate validation record differs")
...
def _frontier_artifacts(
    *,
    ...
    for epoch in frontier:
        ...
        if path.is_symlink() or not path.is_file():
            raise SCTransNetSBSCV31RunnerError("frontier candidate is missing")
        artifacts[epoch] = {
...
def _validate_histories(
    *,
    ...
    if not isinstance(training_history, list) or len(training_history) != completed_epoch:
        raise SCTransNetSBSCV31RunnerError("resume training history is incomplete")
    ...
    for expected_epoch, raw in enumerate(training_history, start=1):
        if not isinstance(raw, Mapping) or raw.get("epoch") != expected_epoch:
            raise SCTransNetSBSCV31RunnerError("resume training history order differs")
        ...
        for field in ("mean_train_loss", "learning_rate"):
            ...
            if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
                raise SCTransNetSBSCV31RunnerError("training history is non-finite")
        if float(record["mean_train_loss"]) < 0.0:
            raise SCTransNetSBSCV31RunnerError("training loss must be non-negative")
    ...
    if not isinstance(validation_history, list):
        raise SCTransNetSBSCV31RunnerError("resume validation history is malformed")
...
def _normalize_artifact_map(value: Any) -> dict[int, dict[str, Any]]:
    if not isinstance(value, Mapping):
        raise SCTransNetSBSCV31RunnerError("candidate artifact map is malformed")
    normalized: dict[int, dict[str, Any]] = {}
    for raw_epoch, raw_artifact in value.items():
        if isinstance(raw_epoch, bool) or not isinstance(raw_epoch, int):
            raise SCTransNetSBSCV31RunnerError("candidate artifact epoch is malformed")
        if not isinstance(raw_artifact, Mapping):
            raise SCTransNetSBSCV31RunnerError("candidate artifact is malformed")
...
def _validate_resume_candidates(
    *,
    ...
    if tuple(sorted(normalized)) != frontier:
        raise SCTransNetSBSCV31RunnerError("resume artifacts differ from dual frontier")
    ...
    for epoch in frontier:
        ...
        if artifact.get("relative_path") != f"candidates/{path.name}":
            raise SCTransNetSBSCV31RunnerError("candidate relative path differs")
        path = _regular_file(path, run_dir=run_dir, label="frontier candidate")
        expected_files.add(path)
        if artifact.get("file_sha256") != _sha256_file(path):
            raise SCTransNetSBSCV31RunnerError("candidate hash differs")
    ...
    if candidate_dir.exists():
        if candidate_dir.is_symlink() or not candidate_dir.is_dir():
            raise SCTransNetSBSCV31RunnerError("candidate directory is not regular")
...
def _prune_after_committed_latest(
    *,
    ...
    for path in candidate_dir.iterdir():
        if path.is_symlink() or not path.is_file():
            raise SCTransNetSBSCV31RunnerError("candidate entry is not regular")
        epoch = _candidate_epoch(path, candidate_dir)
...
def _load_resume_state(
    *,
    ...
    try:
        ...
        raise SCTransNetSBSCV31RunnerError("resume state could not be read safely") from exc
    if (
        not isinstance(payload, Mapping)
        ...
        raise SCTransNetSBSCV31RunnerError("resume run identity differs")
    ...
    if (
        ...
        raise SCTransNetSBSCV31RunnerError("resume epoch is outside the run")
    _load_model_state(
        model,
        payload.get("state_dict"),
        method=str(identity["method"]),
    ...
    try:
        ...
        raise SCTransNetSBSCV31RunnerError("resume RNG state is malformed") from exc
...
def _load_candidate_state(
    *,
    ...
    if epoch not in artifacts:
        raise SCTransNetSBSCV31RunnerError("selected epoch is absent from frontier")
    path = _regular_file(
    ...
    if artifacts[epoch].get("file_sha256") != _sha256_file(path):
        raise SCTransNetSBSCV31RunnerError("selected candidate hash differs")
    record = next(
        (record for record in history if int(record["epoch"]) == epoch), None
    )
    if record is None:
        raise SCTransNetSBSCV31RunnerError("selected validation record is missing")
    _validate_candidate_payload(
        path=path,
        run_dir=run_dir,
        candidate_dir=candidate_dir,
...
def _final_checkpoint_payload(
    *,
    ...
    if role not in zero_selection.VALID_ROLES:
        raise SCTransNetSBSCV31RunnerError("invalid final checkpoint role")
...
def _require_selected_state_hash(
    ...
    if role not in zero_selection.VALID_ROLES:
        raise SCTransNetSBSCV31RunnerError("invalid selected-state role")
    try:
...
def _atomic_write_final_pair(
    *,
    ...
    if set(payloads) != expected_roles:
        raise SCTransNetSBSCV31RunnerError("exactly two final role payloads are required")
    allowed_names = {
    ...
    if unexpected:
        raise SCTransNetSBSCV31RunnerError("unexpected final weight file exists")
    ...
    for role in zero_selection.VALID_ROLES:
        ...
        if (
            ...
            raise SCTransNetSBSCV31RunnerError("final checkpoint verification failed")
        finals[role] = {
    ...
    if actual != allowed_names:
        raise SCTransNetSBSCV31RunnerError("final physical weight set is not exact")
    return finals
...
def _process_lock(path: Path) -> Iterator[None]:
    ...
    if path.is_symlink():
        raise SCTransNetSBSCV31RunnerError("run lock must not be a symlink")
    ...
    try:
        try:
            ...
            raise SCTransNetSBSCV31RunnerError("this run is already active") from exc
        yield
    finally:
    ...
    *,
    ...
    if validation_record is not None:
        if int(validation_record["epoch"]) != epoch:
            raise SCTransNetSBSCV31RunnerError("validation record epoch differs")
        _save_candidate(
            path=_candidate_path(candidate_dir, epoch),
            epoch=epoch,
            state=state,
...
def _run_locked(
    ...
    if len(train_data) < 1 or len(val_data) < 1:
        raise SCTransNetSBSCV31RunnerError("train/validation data is empty")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for images, masks in train_loader:
            ...
            optimizer.step()
            if args.method == "sbsc_v31":
                core.project_sbsc_v31_constraints_(model)
    ...
    if tuple(sorted(candidate_artifacts)) != frontier:
        raise SCTransNetSBSCV31RunnerError("final artifacts differ from dual frontier")
...
def run(args: argparse.Namespace) -> dict[str, Path]:
    if args.architecture_seed != ARCHITECTURE_SEED or args.run_seed != RUN_SEED:
        raise SCTransNetSBSCV31RunnerError("architecture and runtime seed must be 42")
    if args.preflight:
        raise SCTransNetSBSCV31RunnerError(
