...
from experiments.sbsc_v2_selection import (
...
def test_final_requires_exact_500_through_1000_history():
    ...
    with pytest.raises(SBSCV2SelectionError, match="cadence differs"):
...
def test_history_tampering_is_rejected(mutation: str):
    ...
    if mutation == "test_role":
        ...
        bad[0]["epoch"] = 499
    with pytest.raises(SBSCV2SelectionError):
        select_prefix(
...
def test_test_access_disclosure_must_be_explicit_false(value):
    ...
    with pytest.raises(SBSCV2SelectionError, match="explicit false"):
        select_prefix(
...
def test_public_schedule_and_role_alias_mutation_cannot_weaken_final(monkeypatch):
    import experiments.sbsc_v2_selection as module
