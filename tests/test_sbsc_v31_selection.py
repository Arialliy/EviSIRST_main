...
from experiments.sbsc_v31_selection import (
    SBSCV31SelectionError,
    SCHEDULE_SCHEMA,
    SELECTION_SCHEMA,
...
def test_v31_schema_and_method_identity_are_exact() -> None:
    assert SELECTION_SCHEMA == (
        "sctransnet_sbsc_v31_validation_dual_role/v1"
    )
    assert SCHEDULE_SCHEMA == (
        "sctransnet_sbsc_v31_validation_schedule/v1"
    )
    payload = select_prefix(
        [], completed_epoch=0, expected_identity=_IDENTITY
    )
    assert payload["schema"] == SELECTION_SCHEMA
    assert payload["schedule_schema"] == SCHEDULE_SCHEMA
    assert payload["selection_identity"]["method"] == "sbsc_v31"
...
def test_final_requires_exact_500_through_1000_history():
    ...
    with pytest.raises(SBSCV31SelectionError, match="cadence differs"):
...
def test_history_tampering_is_rejected(mutation: str):
    ...
    if mutation == "test_role":
        ...
        bad[0]["epoch"] = 499
    with pytest.raises(SBSCV31SelectionError):
        select_prefix(
...
def test_test_access_disclosure_must_be_explicit_false(value):
    ...
    with pytest.raises(SBSCV31SelectionError, match="explicit false"):
        select_prefix(
...
def test_public_schedule_and_role_alias_mutation_cannot_weaken_final(monkeypatch):
    import experiments.sbsc_v31_selection as module
...
@pytest.mark.parametrize("legacy_method", ("sbsc_v2", "sbsc_v21"))
def test_legacy_identity_and_records_are_rejected(
    legacy_method: str,
) -> None:
    legacy_identity = {**_IDENTITY, "method": legacy_method}
    legacy_record = _record(500, miou=0.61, pd=0.91, fa=2e-5)
    legacy_record["method"] = legacy_method
    with pytest.raises(SBSCV31SelectionError, match="method differs"):
        select_prefix(
            [legacy_record],
            completed_epoch=500,
    ...
    with pytest.raises(SBSCV31SelectionError, match="method differs"):
        select_prefix(
            [legacy_record], completed_epoch=500, expected_identity=_IDENTITY
        )
