...
from experiments.sbsc_v32_selection import (
...
def test_v32_schema_and_method_identity_are_exact() -> None:
    assert SELECTION_SCHEMA == (
        "sctransnet_sbsc_v32_validation_dual_role/v1"
    )
    assert SCHEDULE_SCHEMA == (
        "sctransnet_sbsc_v32_validation_schedule/v1"
    )
    ...
    assert payload["selection_identity"]["method"] == "sbsc_v32"
...
def test_final_requires_exact_500_through_1000_history():
    ...
    with pytest.raises(SBSCV32SelectionError, match="cadence differs"):
...
def test_history_tampering_is_rejected(mutation: str):
    ...
    if mutation == "test_role":
        ...
        bad[0]["epoch"] = 499
    with pytest.raises(SBSCV32SelectionError):
        select_prefix(
...
def test_test_access_disclosure_must_be_explicit_false(value):
    ...
    with pytest.raises(SBSCV32SelectionError, match="explicit false"):
        select_prefix(
...
def test_public_schedule_and_role_alias_mutation_cannot_weaken_final(monkeypatch):
    import experiments.sbsc_v32_selection as module
...
@pytest.mark.parametrize(
    "legacy_method", ("sbsc_v2", "sbsc_v21", "sbsc_v31")
)
def test_legacy_identity_and_records_are_rejected(
    ...
    with pytest.raises(SBSCV32SelectionError, match="method differs"):
        select_prefix(
            [legacy_record],
            completed_epoch=500,
