...
import train_sctransnet_sbsc_v31_validation as runner
...
def test_epoch_prefix_is_empty_before_500_and_formal_schedule_has_501_records() -> None:
    ...
    with pytest.raises(runner.SCTransNetSBSCV31RunnerError):
...
def test_paired_methods_use_exactly_the_same_shuffle_generator_stream() -> None:
    baseline = _args()
    candidate = SimpleNamespace(**{**vars(baseline), "method": "sbsc_v31"})
...
class _NoopCore:
    @staticmethod
    def validate_sbsc_v31_state_dict(state, method):
        return {"method": method}

    @staticmethod
    def validate_sctransnet_sbsc_v31(model, require_zero_gain=False):
        return {}

    @staticmethod
    def project_sbsc_v31_constraints_(model):
        return None
...
def test_out_of_range_gain_is_rejected_before_live_load_state_dict() -> None:
    ...
    with pytest.raises(
        runner.SCTransNetSBSCV31RunnerError, match=r"outside \[0, 0.25\]"
    ):
...
def test_frozen_v31_gain_contract_and_v2_state_are_distinct() -> None:
    ...
    assert runner.EXPECTED_SBSC_V31_STATE_KEY_COUNT == 511
    assert runner.EXPECTED_SBSC_V31_PARAMETER_COUNT == 11_325_943
...
def test_v2_resume_schema_is_rejected_before_live_state_load() -> None:
    ...
    with tempfile.TemporaryDirectory(dir=runs) as temporary:
        ...
        with pytest.raises(
            runner.SCTransNetSBSCV31RunnerError,
...
@pytest.mark.parametrize("method", ("sctransnet", "sbsc_v31"))
def test_one_epoch_cpu_fixture_smoke_writes_two_roles_and_never_opens_test(
    ...
    with tempfile.TemporaryDirectory(dir=runs) as temporary:
        ...
        def fake_build(_args):
            ...
            core.project_sbsc_v31_constraints_ = lambda model: (
