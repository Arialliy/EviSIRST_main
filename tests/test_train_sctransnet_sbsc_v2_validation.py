...
import train_sctransnet_sbsc_v2_validation as runner
...
def test_epoch_prefix_is_empty_before_500_and_formal_schedule_has_501_records() -> None:
    ...
    with pytest.raises(runner.SCTransNetSBSCV2RunnerError):
...
def test_paired_methods_use_exactly_the_same_shuffle_generator_stream() -> None:
    baseline = _args()
    candidate = SimpleNamespace(**{**vars(baseline), "method": "sbsc_v2"})
...
class _NoopCore:
    @staticmethod
    def validate_sbsc_v2_state_dict(state, method):
        return {"method": method}

    @staticmethod
    def validate_sctransnet_sbsc_v2(model, require_zero_gain=False):
        return {}
...
def test_out_of_range_gain_is_rejected_before_live_load_state_dict() -> None:
    ...
    state[runner.GAIN_STATE_KEYS[2]] = torch.tensor(0.250001)
    gain_contract = {
        "state_keys": list(runner.GAIN_STATE_KEYS),
        "bounds": [0.0, 0.25],
    }
    with pytest.raises(
        runner.SCTransNetSBSCV2RunnerError, match=r"outside \[0, 0.25\]"
    ):
...
def test_nonfinite_live_adam_moment_is_rejected_before_any_commit_write() -> None:
    ...
    with tempfile.TemporaryDirectory(dir=runs) as temporary:
        run_dir = Path(temporary)
        with pytest.raises(
            runner.SCTransNetSBSCV2RunnerError,
...
def test_one_epoch_cpu_fixture_smoke_writes_two_roles_and_never_opens_test() -> None:
    runs = runner.PROJECT_ROOT / "runs"
    runs.mkdir(exist_ok=True)
    access_log: list[str] = []
    manifest = {
    ...
    with tempfile.TemporaryDirectory(dir=runs) as temporary:
        ...
        def fake_build(_args):
            model = _SixHeadFixture()
            return (
                model,
                {"method": "sctransnet", "state_key_count": 1},
                SimpleNamespace(__file__=runner.__file__),
                {
                    "state_key_count": 1,
                    "parameter_count": 1,
                    "ordered_state_keys_sha256": "d" * 64,
                    "tensor_schema_sha256": "e" * 64,
                    "gain": {"state_keys": [], "bounds": None},
                },
                (),
            )
