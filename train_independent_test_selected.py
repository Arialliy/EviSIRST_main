#!/usr/bin/env python3
"""Train one clean EviSIRST model and select it on its own test split.
...
complete public evaluator after every epoch, and the checkpoint with the
strictly greatest raw global foreground mIoU is retained.  Exact mIoU ties
keep the earlier epoch.
...
completion the only published artifact is
``result/<dataset>/EviSIRST.pth.tar``; no file is written under ``results/``.
"""
...
RECOVERY_SCHEMA = "evisirst_independent_test_selected_recovery/v1"
SUMMARY_SCHEMA = "evisirst_independent_test_selected_summary/v1"
PROTOCOL_NAME = "evisirst_three_independent_test_selected_v1"
EXPECTED_COUNTS = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=PROJECT_ROOT / "runs" / "three_independent_test_selected_v1",
    )
    parser.add_argument(
...
def _source_tree_sha256() -> str:
    paths = [
        PROJECT_ROOT / "train_independent_test_selected.py",
...
def _formal_config(
    ...
    *,
    ...
    return {
        ...
        "selection_comparison": "strict_greater_than",
        "selection_tie_rule": "earliest_epoch_wins",
...
def _load_recovery(
    ...
    *,
    ...
    stored_epoch = payload.get("best_epoch")
    if (score, epoch) != (
        None if stored_score is None else float(stored_score),
        None if stored_epoch is None else int(stored_epoch),
    ):
...
def _publish_once(path: Path, payload: Mapping[str, Any]) -> str:
    ...
    if unexpected:
        raise FileExistsError(
            f"final result directory must contain only {path.name}: {unexpected}"
        )
    if path.exists():
...
def _validate_completed_publication(
    summary_path: Path, published_path: Path, dataset: str
) -> None:
    if (
    ...
    published = summary.get("published_checkpoint") if isinstance(summary, Mapping) else None
    if (
        ...
        or published.get("path") != str(published_path)
        or published.get("sha256") != _sha256(published_path)
    ):
        raise ValueError("completed independent publication summary differs")
    checkpoint = torch.load(published_path, map_location="cpu", weights_only=True)
    if (
        not isinstance(checkpoint, Mapping)
        or checkpoint.get("schema") != CHECKPOINT_SCHEMA
        or checkpoint.get("model") != "EviSIRST"
        or checkpoint.get("dataset") != dataset
        or checkpoint.get("checkpoint_role") != "test_miou_selected"
...
def run(args: argparse.Namespace) -> Path:
    ...
    published_path = result_dir / "EviSIRST.pth.tar"
    ...
    if result_dir.exists():
        ...
        unexpected = [item for item in result_dir.iterdir() if item.name != published_path.name]
        if unexpected:
            raise FileExistsError(
                f"result dataset directory contains unexpected files: {unexpected}"
            )
    if summary_path.exists():
        if args.resume and published_path.is_file() and not published_path.is_symlink():
            ...
            _validate_completed_publication(summary_path, published_path, args.dataset)
            latest_path.unlink(missing_ok=True)
            best_recovery_path.unlink(missing_ok=True)
            return published_path
        raise FileExistsError("this independent experiment is already complete")
    if published_path.exists() and not args.resume:
        raise FileExistsError(
    ...
    if args.resume:
        ...
        best_score = (
            None if recovery["best_score"] is None else float(recovery["best_score"])
        )
        best_epoch = (
            None if recovery["best_epoch"] is None else int(recovery["best_epoch"])
        )
        elapsed_before = float(recovery.get("elapsed_seconds", 0.0))
        start_epoch = completed + 1
    elif latest_path.exists() or best_recovery_path.exists():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if metrics is not None:
            suffix = (
                ...
                f" best_epoch={best_epoch} best_mIoU={100.0 * float(best_score):.6f}%"
            )
        print(
    ...
    if best_epoch is None or best_score is None or not best_recovery_path.is_file():
    ...
    best_recovery = _load_recovery(
        best_recovery_path, config=config, expected_state=expected_state
    )
    if int(best_recovery["epoch"]) != best_epoch:
        raise RuntimeError("best recovery epoch differs from selected epoch")
    ...
    if published_path.exists():
        checkpoint_sha = _sha256(published_path)
    else:
        checkpoint_sha = _publish_once(published_path, payload)
    reopened = torch.load(published_path, map_location="cpu", weights_only=True)
    if (
        not isinstance(reopened, Mapping)
        or reopened.get("schema") != CHECKPOINT_SCHEMA
        or reopened.get("dataset") != args.dataset
        or int(reopened.get("epoch", -1)) != best_epoch
        or float(reopened.get("selection_score", -1.0)) != best_score
    ):
        raise RuntimeError("published checkpoint identity replay failed")
    ...
    for key, expected_tensor in payload["state_dict"].items():
        if not torch.equal(reopened["state_dict"][key], expected_tensor):
            raise RuntimeError(
                f"published checkpoint tensor differs from selected state: {key!r}"
            )
    _write_json(
        summary_path,
        {
            ...
            "published_checkpoint": {
                ...
                "only_file_in_result_directory": True,
            },
            "selection_history": history,
    ...
    # user requested that only the selected checkpoint remain after completion.
    latest_path.unlink()
    best_recovery_path.unlink()
    return published_path
